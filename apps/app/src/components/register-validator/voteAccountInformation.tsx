...
export default function VoteAccountInformation({
  ...
  const initialValues: VoteAccountInfo = voteAccountInfo ?? {
    ...
    init_commission: 0,
    nft_holders_share: 0,
    initial_redemption_fee: 0,
    redemption_fee_duration: 0,
    is_validator_id_switchable: false,
  };

  const validationSchema = Yup.object().shape({
    max_primary_stake: Yup.number().required('required'),
    init_commission: Yup.number().required('required'),
    nft_holders_share: Yup.number().required('required'),
    initial_redemption_fee: Yup.number(),
    redemption_fee_duration: Yup.number().required('required'),
